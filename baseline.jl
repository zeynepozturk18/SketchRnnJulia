...
function build_model(hps)
    ...
    if hps["conditional"]
        ...
        # KL cost
        kl_cost = -0.5*(1+presig-mean^2-exp(presig))
...
function baseline_sample(mu, sigma, seq_len=250)
    strokes = zeros(Float32, seq_len, 3)
    result = zeros(Float32, seq_len, 5)

    strokes[:,1:2] = randn(Float32, (seq_len, 2))
    strokes[:,3] = floor.(rand(0.2:0.01:1.2, seq_len))
    # make it stroke-5
    result[:, 1:2] = strokes[:, 1:2]
    result[:, 4] = strokes[:, 3]
    result[:, 3] = 1 .- result[:, 4]

    # put in the first token, as described in sketch-rnn methodology
    result[2:end, :] = result[1:end .!= end, :]
    result[1, :] .= 0
    result[1, 3] = 1 # setting S_0 from paper.
    result[1, 4] = 0 # setting S_0 from paper.
    result[1, 5] = 0 # setting S_0 from paper.
    result[end, 5] = 1
